...
import { Container, TextField, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Switch, InputAdornment, Box, Dialog, DialogTitle,
DialogContent, DialogActions, Typography, Snackbar, Alert } from '@mui/material';
...
const Cursos = () => {
  ...
  const fetchUsuariosInscritos = async (cursoId) => {
    try {
      const response = await fetch(`http://localhost:3000/api/cursos/${cursoId}/usuarios`);
  ...
  const handleDeleteCurso = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/cursos/${selectedCurso._id}`, {
        method: 'DELETE',
      });
    ...
    <Container>
          ...
          <TableBody>
            {cursos.map((curso) => (
              <TableRow key={curso.id}>
                ...
                <TableCell>
                  <img src={curso.imagen} alt="Imagen del curso" width="50" onError={(e) => e.target.src = defaultImage} />
                </TableCell>
      ...
      <Dialog open={openInfoDialog} onClose={handleCloseInfoDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          {selectedCurso && (
            <Box>
              <Typography variant="h6">Título: {selectedCurso.titulo}</Typography>
      ...
      <Dialog open={openDeleteDialog} onClose={handleCloseDeleteDialog} maxWidth="xs" fullWidth>
        ...
        <DialogContent dividers>
          <Typography>¿Estás seguro de que quieres eliminar el curso <strong>{selectedCurso?.titulo}</strong>?</Typography>
        </DialogContent>
        <DialogActions>
